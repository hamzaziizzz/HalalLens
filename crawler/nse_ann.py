...
def main():
    """Example usage of NSE announcements fetcher."""
    fetcher = NSEAnnouncementsFetcher()

    try:
        # Get current date and yesterday for demo
        today = date.today().strftime('%Y-%m-%d')
        yesterday = date.today().replace(day=date.today().day-2).strftime('%Y-%m-%d')

        # Fetch announcements
        announcements = fetcher.get_corporate_announcements(yesterday, today)

        if announcements:
            print(announcements)
            # # Save to cache
            # timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            # fetcher.save_to_cache(announcements, f"nse_announcements_{timestamp}.json")
            #
            # # Print summary
            ...
            #         print(f"{i+1}. {ann.get('symbol', 'N/A')} - {ann.get('subject', 'N/A')}")
        else:
